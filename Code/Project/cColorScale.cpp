...
bool cColorScale::insert(double position, sf::Color color)
{
	std::pair< cColorScale::iterator,bool > ret = std::map<double,sf::Color>::insert(std::make_pair(position,color));
	return ret.second;
}
...
void cColorScale::fillTab(sf::Color* colorTab, int size,InterpolationFunction::InterpolationFunction function) const
{
	
	cColorScale::const_iterator start = std::map<double,sf::Color>::begin();
	cColorScale::const_iterator last  = std::map<double,sf::Color>::end();
	last--;
		
	double pos = 0.0;
	double distance = last->first - start->first;
	cColorScale::const_iterator it =  start;
